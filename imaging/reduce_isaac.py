...
parser.add_option('-d','--detector',dest='detector',
       help='detector name',type='string',
       default='ISAAC')
...
files = glob.glob('{0}*.fits'.format(options.detector))
...
for file in files:
    ...
    objects.append(hdulist.header[0]['OBJECT'])
    filters.append(hdulist.header[0]['HIERARCH ESO INS FILT1 ID'])

# Header changes
rmkeys = ['PROJP1','PROJP3','PV1_1','PV1_2','PV2_1','PV2_2']

if options.dobias == 1:
    biases  = glob.glob('bias/{0}*.fits'.format(options.detector))
    nbias   = len(biases)
    print nbias
...
if options.doflats == 1:
    allflats = np.array(glob.glob('flat/{0}*.fits'.format(options.detector))
...
if options.standards == 1:
    ...
    field = 'std'
    print field
    files = glob.glob('{0}/{1}*.fits'.format(field,options.detector))
    nfiles = len(files)
    print files
...
if options.runsex == 1:
    for field in fields:
        pfiles = glob.glob('{0}/p{1}.????-??-??T??:??:??.???.fits'.format(field,options.detector))
        print pfiles
        for pfile in pfiles:
